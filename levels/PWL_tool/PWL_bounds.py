...
class PWLPlot(pg.PlotWidget):
    ...
    def mouseMoveEvent(self, ev):
        if self.active_pts:
            ...
            self.temp_line = pg.PlotDataItem([start[0], data_pt.x()], [start[1], data_pt.y()],
                                             pen=pg.mkPen(COLOR_RED, width=2, style=Qt.PenStyle.DashLine))
    ...
    def add_node(self, x, y, is_active=False):
        ...
        color = COLOR_RED if is_active else COLOR_YELLOW
    ...
    def commit_segment(self, x, y):
        # Convert the 'current' red cross to yellow ## gray
        for item in self.active_items:
            if isinstance(item, pg.PlotDataItem) and item.opts.get('symbol') == "+":
                item.setSymbol("d")
                item.setSymbolBrush(COLOR_YELLOW)
                item.setSymbolPen(COLOR_YELLOW)

        # Add the line
        prev_x, prev_y = self.active_pts[-1]
        line = pg.PlotDataItem([prev_x, x], [prev_y, y], pen=pg.mkPen(COLOR_YELLOW, width=1))
        self.addItem(line)
    ...
    def finalize(self):
        ...
        self.addItem(pg.PlotDataItem([self.active_pts[0][0]], [self.active_pts[0][1]],
                                     symbol='p', symbolBrush=(COLOR_YELLOW), symbolSize=15))
        self.addItem(pg.PlotDataItem([self.active_pts[-1][0]], [self.active_pts[-1][1]],
                                     symbol='h', symbolBrush=(COLOR_YELLOW), symbolSize=15))
...
def Manual_PWL_bounds(fgSaveFigures=False, outFileName="Iris_Classifier_Bounds"):
    app = QApplication(sys.argv)
    canvas_widget = ManualClassifierGUI()
    canvas_widget.show()
    ...
    if fgSaveFigures:
        image_path = '/'.join(("images", outFileName)) + ".png"
        pixmap = canvas_widget.grab()
        pixmap.save(image_path)
        print(f"Canvas saved to: {outFileName} [{image_path}]")
